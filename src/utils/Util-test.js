...
describe('Util', function () {
  ...
  time="2015-04-17T21:43:47-04:00" level="debug" msg="executing: /usr/bin/VBoxManage showvminfo dev2 --machinereadable"`;
      expect(util.removeSensitiveData(testdata).indexOf('CERTIFICATE')).to.equal(-1);
      expect(util.removeSensitiveData(testdata).indexOf('nX5Vh1bslYJzsSdBNZFWSKShZ+gtRpjtV7NynANDJPQNIRhDxAf4uDY9hA2c')).to.equal(-1);
      expect(util.removeSensitiveData(testdata).indexOf('<redacted>')).to.not.equal(-1);
    });
  ...
  time="2015-04-17T21:43:47-04:00" level="debug" msg="executing: ssh -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectionAttempts=30 -o LogLevel=quiet -p 50483 -i /Users/johnappleseed/.docker/machine/machines/dev2/id_rsa docker@localhost echo \"-----BEGIN CERTIFICATE-----\nMIIC+DCCAeKgAwIBAgIRANfIbsa2M94gDY+fBiBiQBkwCwYJKoZIhvcNAQELMBIx\nEDAOBg`;
      expect(util.removeSensitiveData(testdata).indexOf('PRIVATE')).to.equal(-1);
      expect(util.removeSensitiveData(testdata).indexOf('94fYH4W1Fq0uDJ9kZ7oItLyF5EPaLlY9E8+YuJBl0OSTtdicROUv')).to.equal(-1);
      expect(util.removeSensitiveData(testdata).indexOf('<redacted>')).to.not.equal(-1);
    });
    ...
    it('filters username data', function () {
      ...
      expect(util.removeSensitiveData(testdata).indexOf('/Users/johnappleseed/')).to.equal(-1);
      expect(util.removeSensitiveData(testdata).indexOf('/Users/<redacted>/')).to.not.equal(-1);
    });

    it ('returns input if empty or not a string', function () {
      expect(util.removeSensitiveData('')).to.equal('');
      expect(util.removeSensitiveData(1)).to.equal(1);
      expect(util.removeSensitiveData(undefined)).to.equal(undefined);
    });
  });
});
